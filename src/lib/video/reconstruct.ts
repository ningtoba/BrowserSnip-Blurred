...
import { reconstructCommandRaw, concatCommand } from '@/lib/ffmpeg/commands';
...
export async function reconstructVideoRaw(
  totalFrames: number,
  metadata: VideoMetadata,
  onProgress?: (phase: string, percent: number) => void
): Promise<Blob> {
  const ffmpeg = await getFFmpeg();
  const { fps, width, height } = metadata;

  if (totalFrames <= BATCH_SIZE) {
    onProgress?.('encoding', 50);
    await ffmpeg.exec(
      reconstructCommandRaw('frame_%04d.rgba', 'output.mp4', fps, width, height),
      300_000
    );
  ...
  for (let seg = 0; seg < numSegments; seg++) {
    ...
    for (let f = startFrame; f <= endFrame; f++) {
      try {
        await ffmpeg.deleteFile(`frame_${String(f).padStart(4, '0')}.rgba`);
      } catch { /* ignore */ }
    }
  }
